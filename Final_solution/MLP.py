...
def rmse(y_true, y_pred):
    """
    Root Mean Squared Error: the evaluation metric used in this challenge
    This function is used as our loss function for the MLP
    """
    return K.sqrt(K.mean(K.square(y_pred - y_true)))

def callbacks(lr_factor, lr_patience, es_patience, tensorboard=False):
    """ 
    Defines the callbacks used in the neural network.
    Arguments are learning rate factor, learning rate and early stopping patience and a boolean whether to include tensorboard
    """
...
def optimizer(lr, amsgrad=True):
    """
    Defining the optimizer to use in the MLP.
    Arguments are learning rate and a boolean to use amsgrad or not
    """
    return tfa.optimizers.AdamW(
        weight_decay=0.0000004, learning_rate=lr, beta_1=0.95, amsgrad=amsgrad
    )

def nn_model(X_train, lr=0.001, dropout_pct=0.1):
    """
    Defining the MLP
    Arguments needed are X_train (for input shape), learning rate and dropout percent
